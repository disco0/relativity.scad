...
function ensureArrOfArr(arr) = is_undef(arr.x) || is_list(arr.x) ? arr : [ arr ];
...
module translated(offset, n=[1], class="*"){
  offsets = ensureArrOfArr(offset);
  for(_offset=offsets)
  {
        show(class)
        for(i=n)
            _translate(_offset*i)
...
module rotated(offsets, n=[1], class="*"){
  offsets = ensureArrOfArr(offsets);
  for(_offset=offsets)
  {
        show(class)
        for(i=n)
...
module align(anchors, bounds="box"){
  anchors = ensureArrOfArr(anchors);
  for(anchor=anchors)
  {
    if(bounds == "box")
...
module box(
  ...
  _assign($parent_size = _size,
      $parent_type="box",
      $parent_bounds=[_size.x < indeterminate/2? _size.x : 0,
              _size.y < indeterminate/2? _size.y : 0,
              _size.z < indeterminate/2? _size.z : 0],
      $parent_radius=sqrt(pow(_size.x/2,2) + pow(_size.y/2,2) + pow(_size.z/2,2)),
